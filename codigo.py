...
def processar_analise_qualitativa(texto_analise):
    ...
    # Tentar identificar seções por padrões
    for secao_key, config in secoes_config.items():
        # Procurar por padrões como "**Pontos Fortes:**" ou "- **Pontos Fortes:**"
        patterns = [
            f"\\*\\*{config['titulo']}:\\*\\*\\s*([^*]+?)(?=\\*\\*|$)",
            f"- \\*\\*{config['titulo']}:\\*\\*\\s*([^*]+?)(?=\\*\\*|$)",
            f"{config['titulo']}:\\s*([^\\n]+(?:\\n(?!\\*\\*)[^\\n]*)*)"
        ...
        for pattern in patterns:
            match = re.search(pattern, texto_analise, re.IGNORECASE | re.DOTALL)
            if match:
